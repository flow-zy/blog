...
用 `new Student()` 创建的对象还从原型上获得了一个 `varructor` 属性，它指向函数 `Student` 本身:

```js
xiaoming.varructor === Student.prototype.varructor; // true
Student.prototype.varructor === Student; // true
...
红色箭头是原型链。注意，`Student.prototype` 指向的对象就是 `xiaoming`、`xiaohong` 的原型对象，这个原型对象自己还有个属性 `varructor`，指向 `Student` 函数本身。
